...
module.exports.forgetpass = async (req, res) => {
  ...
  const token = jwt.sign({ user: user._id },'forgot secret' ,{expiresIn: '30m'}) 
  const user2 = await User.findOneAndUpdate(email,{resetlink: token});
   const msg = {
      to: user.email, // Change to your recipient
      from: 'mohamedmohesn120@gmail.com', // Change to your verified sender
      subject: 'this mail for reset the password',
      text: 'click to reset',
      html: `<button><a href="https://chat-kit--app.herokuapp.com/${token}" >click to reset</a></button>`, }
  try {
...
module.exports.resetpass = async (req, res) => {
  ...
  try {
    jwt.verify(resetlink,'forgot secret',function name(params) {
